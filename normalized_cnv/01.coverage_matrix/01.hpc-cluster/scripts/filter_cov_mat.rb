...
class Region
  ...
  mat_io = Zlib::GzipReader.open(cov_path)
  ...
  while true
          begin
            	#puts "BED: #{bed.to_r}"
                while not bed.overlap(mat) #Advance mat table
                  	#puts "#{mat.to_r} < #{bed.to_r}"
                  	mat_line = mat_each.next
			mat = Region.mat(mat_line)
                        j += 1
		end
 		
		while bed.overlap(mat)
                  	#puts "#{bed.to_r} contains #{mat.to_r} "
                  	out.puts mat_line
			mat_line = mat_each.next
			mat = Region.mat(mat_line)
			i += 1
		end
                bed = Region.bed(bed_each.next) #if mat.end > bed.end
		#break if i > 10 and j > 10
